var Util = 
{
	...
	createJSONArrayFromRSS:function(xml)
	{
		...
		for (var i = 0; i < items.length; i++)
		{
			...
			if(item.getElementsByTagName("title")[0].childNodes[0].nodeValue=="Squaw Valley"){
				myArray.Last24hrs = 0;
			}
			...
			//alert(myserializer.serializeToString(xml));
			alert(item.getElementsByTagName("title")[0].childNodes[0].nodeValue);
			...
			if((item.getElementsByTagName("title")[0].childNodes[0].nodeValue=="Squaw Valley") ||
				(item.getElementsByTagName("title")[0].childNodes[0].nodeValue=="Vail") ||
				...
				(item.getElementsByTagName("title")[0].childNodes[0].nodeValue=="Heavenly"))
			{
				...
				alert(xml.getElementsByTagName("pubDate")[0].childNodes[0].nodeValue);
